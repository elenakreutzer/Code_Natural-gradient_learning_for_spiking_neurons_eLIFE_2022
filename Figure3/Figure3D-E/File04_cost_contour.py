...
###############################################################################   
cost=np.zeros((int((wmax-wmin)/precisioncost)+1,int((wmax-wmin)/precisioncost)+1))     
for i,w1i in enumerate(np.arange(wmin,wmax,precisioncost)):
    for j,w2j in enumerate(np.arange(wmin,wmax,precisioncost)):
        ...
        cost[i,j]=((np.log(rho(Vopt))-np.log(rho(V)))*maxfire*rho(Vopt) +maxfire*(rho(V)-rho(Vopt)))*dt 

        
np.save("2019-01-07_cost"+str(t),cost)        
